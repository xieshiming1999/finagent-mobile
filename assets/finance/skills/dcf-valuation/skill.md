...
od:
  ...
  preview:
    type: markdown
  outputs:
    primary: memory/reports/<safe-company-or-ticker>-dcf.md
...
(`https://github.com/virattt/dexter`). It runs in FinAgent and uses FinAgent
tools/data sources only.

## Goal

Create a reusable Markdown valuation report at:

```text
memory/reports/<safe-company-or-ticker>-dcf.md
```
...
- Use user-provided financial data, uploaded filings, WindMcp when configured,
  MarketData, Research, WebFetch, or public sources the agent can access.
...
5. Estimate discount rate:
   - Use `bundle/skills/dcf-valuation/references/sector-wacc.md` for the
     starting sector range.
...
Write one Markdown file at `memory/reports/<safe-company-or-ticker>-dcf.md`.
Use this structure:
...
path so the user can reopen or reuse it.
