...
''',
  ...
  test('ETF quote-only evidence blocks rank without K-line rows', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(
        role: Role.user,
        content:
            'structured ETF rotation\n'
            'data: {"workflowState":{"contract":"finance-workflow-state-v1","workflowKind":"strategy_design","assetClass":"fund","intentMode":"analysis","executionMode":"preview_only","safetyBoundary":"ETF observation only","evidenceRefs":["quote"],"confirmationState":"none","source":"agent-structured-intent"}}',
      ),
      Message(
        role: Role.assistant,
        content: '',
          ...
          content: '''
{
  "action": "quote",
  "data": [
    {"code": "510300", "name": "沪深300ETF华泰柏瑞", "price": 4.826, "changePct": -1.05}
  ]
}
''',
        ),
    ...
    expect(interception, isNotNull);
    expect(interception!.answer, contains('场内报价证据'));
    expect(interception.answer, contains('未取得 ETF K-line'));
    expect(interception.skippedReason, contains('requires K-line rows'));
  });

  test('ETF quote-only evidence blocks direct K-line expansion', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(
        role: Role.user,
          ...
          content: '''
{
  "action": "quote",
  "data": [
    {"code": "510300.SH", "name": "沪深300ETF华泰柏瑞", "price": 4.826, "changePct": -1.05}
  ]
}
''',
        ),
    ...
    expect(interception, isNotNull);
    expect(interception!.answer, contains('场内报价证据'));
    expect(interception.answer, contains('已停止后续 ETF K-line'));
    expect(interception.skippedReason, contains('requires K-line rows'));
  });

  test('portfolio monitor artifact blocks custom strategy read drift', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(
        role: Role.user,
        content:
            'structured monitor review\n'
            'data: {"workflowState":{"contract":"finance-workflow-state-v1","workflowKind":"monitor_review","assetClass":"portfolio","intentMode":"review","executionMode":"preview_only","safetyBoundary":"review only","evidenceRefs":["monitor"],"confirmationState":"none","source":"agent-structured-intent"}}',
      ),
      Message(
        role: Role.assistant,
        content: '',
        toolUses: const [
          ToolUse(id: 'monitors', name: 'MonitorList', input: {}),
        ],
      ),
      Message(
        role: Role.tool,
...
[ok] workflow_portfolio_rank_monitor_v1_组合再平衡复核_v2 (id: mon_1, interval: 30m, lastRun: 08:44:26)
  strategyId: workflow_portfolio_rank_monitor_v1

monitorList:{"contract":"monitor-list-v1","count":1,"monitors":[{"id":"mon_1","name":"workflow_portfolio_rank_monitor_v1_组合再平衡复核_v2","enabled":true,"intervalMinutes":30,"displayType":"value_card","strategyId":"workflow_portfolio_rank_monitor_v1","template":"portfolio_rebalance_monitor","portfolioEvidence":{"strategyId":"workflow_portfolio_rank_monitor_v1","selectedCount":3,"tradeBoundary":"review_only"},"rebalanceDraft":{"strategyId":"workflow_portfolio_rank_monitor_v1","rebalanceInterval":"monthly","maxPositionWeight":0.4,"tradeBoundary":"review_only","positions":[{"symbol":"600519","targetWeight":0.33},{"symbol":"000858","targetWeight":0.33},{"symbol":"300059","targetWeight":0.34}]}}]}
''',
        ),
      ),
    ];
    ...
    expect(interception, isNotNull);
    expect(interception!.answer, contains('MonitorList'));
    expect(interception.answer, contains('monitor artifact'));
    expect(interception.answer, contains('600519'));
    expect(interception.skippedReason, contains('monitor artifact'));
  });

  test('monitor review state drops file-read tool calls', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(
        role: Role.user,
    ...
    final rewritten = hooks.rewriteToolCalls(
      ...
      toolCalls: const [
        ToolUse(
          id: 'read',
          name: 'Read',
          input: {'file_path': 'memory/pages/market.html'},
        ),
        ToolUse(
          id: 'write',
          name: 'Write',
          input: {'file_path': 'memory/pages/monitor.md', 'content': 'x'},
        ),
        ToolUse(id: 'plan', name: 'EnterPlanMode', input: {}),
        ToolUse(
          id: 'list',
          name: 'MarketData',
          input: {'action': 'custom_strategy_list'},
    ...
    expect(rewritten.map((call) => call.name), ['MarketData']);
  });

  test('monitor review state deduplicates same-batch monitor creation', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(
        role: Role.user,
    ...
    expect(rewritten.single.id, 'create-a');
  });

  test('portfolio monitor context asks structured strategy selection', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(role: Role.user, content: '需要组合监控。'),
      Message(
        role: Role.assistant,
        content: '',
        toolUses: const [
          ...
          ToolUse(id: 'portfolio', name: 'Portfolio', input: {}),
          ToolUse(id: 'monitors', name: 'MonitorList', input: {}),
        ],
      ),
      Message(
        role: Role.tool,
        toolResult: ToolResult(
          toolUseId: 'strategies',
          content: jsonEncode({
            'action': 'custom_strategy_list',
            'runnableStrategies': [
              {
                'strategyId': 'single_v1',
                'name': 'single',
                'symbols': ['600519'],
              },
              {
                'strategyId': 'portfolio_v1',
      ...
      Message(
        ...
        toolResult: ToolResult(
          toolUseId: 'monitors',
          content:
              '0 monitor(s):\n\nmonitorList:{"contract":"monitor-list-v1","count":0,"monitors":[]}',
        ),
      ),
    ];
    ...
    expect(calls, isNotNull);
    expect(calls!.single.name, 'AskUserQuestion');
    expect(calls.single.input.toString(), contains('portfolio_v1'));
    expect(calls.single.input.toString(), isNot(contains('single_v1')));
  });

  test('portfolio strategy answer triggers rank preflight', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(role: Role.user, content: '需要组合监控。'),
      Message(
        ...
        toolUses: const [
          ...
          ToolUse(id: 'ask', name: 'AskUserQuestion', input: {}),
        ],
      ),
      Message(
        role: Role.tool,
      ...
      Message(
        ...
        toolResult: ToolResult(
          toolUseId: 'ask',
          content:
              'User has answered your questions.\naskUserQuestion:{"contract":"ask-user-question-v1","answers":[{"question":"策略","answer":"1","structuredAnswer":{"selectedOptionIndex":1,"selectedOptionLabel":"portfolio_v1"}},{"question":"规则","answer":"1","structuredAnswer":{"selectedOptionIndex":1,"selectedOptionLabel":"固定权重"}}]}',
        ),
      ),
    ];
    ...
    expect(calls, isNotNull);
    expect(calls!.single.name, 'MarketData');
    expect(calls.single.input['action'], 'custom_strategy_rank');
    expect(calls.single.input['strategyId'], 'portfolio_v1');
  });

  test('portfolio rank evidence triggers monitor create preflight', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(role: Role.user, content: '需要组合监控。'),
      Message(
        ...
        toolResult: ToolResult(
          ...
          content: jsonEncode({
            ...
            'rebalanceDraft': {
              'mode': 'equal_weight_top_n',
              'positions': [
                {'symbol': '600519', 'targetWeight': 0.3333},
                {'symbol': '000858', 'targetWeight': 0.3333},
                {'symbol': '300059', 'targetWeight': 0.3333},
              ],
            },
          }),
        ),
    ...
    expect(calls, isNotNull);
    expect(calls!.single.name, 'MonitorCreate');
    expect(calls.single.input['template'], 'portfolio_rebalance_monitor');
    expect(calls.single.input['strategyId'], 'portfolio_v1');
    expect(calls.single.input['interval'], '1d');
  });

  test('monitor creation evidence stops repeated monitor creation', () {
    final hooks = FinanceWorkflowHooks(isBypassTool: (_) => false);
    final messages = [
      Message(
        role: Role.user,
      ...
      Message(
        ...
        toolResult: ToolResult(
          toolUseId: 'create',
          content:
              'Monitor "portfolio_rank_v1_组合再平衡复核" created (id: mon_1).\nTemplate: portfolio_rebalance_monitor',
        ),
      ),
    ];
    ...
    expect(interception, isNotNull);
    expect(interception!.answer, contains('已创建监控'));
    expect(interception.skippedReason, contains('MonitorCreate'));
  });
}
